"""``wenta`` command-line interface.

Sub-commands:

    wenta solve  PATH              load a YAML/JSON network, solve, print results
    wenta info   PATH              load and report structure (no solve)
    wenta validate PATH            schema-validate the file; exit non-zero on error
...
def _format_solved(network, fmt: str, dp: float) -> str:
    ...
    if fmt == "markdown":
        lines = [
            f"# {network.name or 'wenta network'}",
            "",
