"""wenta — ductwork sizing & pressure-drop calculations.

Public API:

>>> from wenta import Network, Source, RigidDuct, Terminal, Round, solve
>>> net = Network("supply")
...
# Must run before any module that imports wentamojo.ext — that's why every
...
# at wentamojo/_mojo_libs/numojo.mojopkg and Mojo's default search path doesn't
...
_mojo_run.subprocess_run_mojo = _make_subprocess_run_wrapper(
    str(_Path(_repo_root) / "wentamojo" / "_mojo_libs"),
