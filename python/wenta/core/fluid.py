...
def air_at_altitude(altitude_m: float, temperature_c: float = 20.0) -> Fluid:
    ...
    Math runs in ``wentamojo.core.fluid.air_at_altitude`` (ISA standard
    atmosphere + Sutherland's viscosity formula). This Python wrapper
    constructs the Python ``Fluid`` so the public API is unchanged.
    """
    from wentamojo.ext.leaf_ext import air_at_altitude as _air_at_altitude
    try:
        density, dynamic_viscosity = _air_at_altitude(altitude_m, temperature_c)
