"""Numerical parity tests: Mojo port vs. Python reference (`wenta`).
...
Run with the project's uv venv active so the `wenta` Python package is
importable:
...
from wentamojo.core.fluid import air_at_altitude
...
from wentamojo.physics.flex import stretch_correction_factor
from wentamojo.physics.losses import straight_pressure_drop, local_pressure_drop
from wentamojo.data.standard_sizes import nearest_round_size
from wentamojo.sizing import (
...
from wentamojo.units import cfm_to_m3s, inwc_to_pa, ft_to_m, air_changes_per_hour
...
def test_parity_round_geometry() raises:
    var py_geo = Python.import_module("wenta.core.geometry")
...
def test_parity_air_at_altitude() raises:
    var py_fl = Python.import_module("wenta.core.fluid")
    var cases = [
...
def test_parity_reynolds() raises:
    var py_fr = Python.import_module("wenta.physics.friction")
    var nu = 1.5e-5
    var cases = [
...
def test_parity_straight_pressure_drop() raises:
    var py_ls = Python.import_module("wenta.physics.losses")
    var cases = [
...
def test_parity_units() raises:
    var py_u = Python.import_module("wenta.units")
...
def test_parity_stretch_correction_factor() raises:
    var py_flex = Python.import_module("wenta.physics.flex")
    var cases = [
...
def test_parity_rectangular_elbow() raises:
    var py_fits = Python.import_module("wenta.components.fittings_library")
    var cases = [
        (0.4, 0.3, 0.2, 90.0),
        (0.4, 0.3, 0.6, 90.0),
        (0.6, 0.2, 0.3, 90.0),
...
def test_parity_nearest_round_size() raises:
    var py_data = Python.import_module("wenta.data.standard_sizes")
